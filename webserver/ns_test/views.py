...
from ns_test import models  # noqa
import json
import logging
...
sys.path.append(r"../")
from ping import ping_test  # noqa: E402
...
def home(request):
    if request.method == 'GET':
        ...
        if update_running:
            return render(request, 'ipshow/index.html',
                          {'baidu': ip_list[0], 'ip_list': ip_list[1:], 'update_running': update_running})
        return render(request, 'ipshow/index.html',
                      {'baidu': ip_list[0], 'ip_list': ip_list[1:], 'update_running': update_running})
...
def insert_comment(request):
    if request.method == 'POST':
        xhr_dict = parse_request_body(request.body.decode())
