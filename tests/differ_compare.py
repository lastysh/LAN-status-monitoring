...
sys.path.append("../")
from ping.ping_test import file_parser  # noqa: E402
...
def check_modify(ss, nr, _or):
    ...
    for ip in ss:
        ...
        if new_mac != old_mac:  # mac
            ...
            record_mac.append(
                "<font color='red'>&lt;M&gt;</font> {0}："
                "<font color='green'>mac地址</font>从 {1} 更改为 {2}".format(ip, new_mac, old_mac))
        if new_name != old_name:  # name
            # print(">> {0}: 名称从 {1} 更改为 {2}".format(ip, new_name, old_name))
            record_name.append(
                "<font color='red'>&lt;M&gt;</font> {0}："
                "<font color='green'>名称</font>从 {1} 更改为 {2}".format(ip, new_name, old_name))
...
def diff_compare():
    global no_use
    try:
        new_rtb, old_rtb = get_route_table()
    except Exception:
        return
